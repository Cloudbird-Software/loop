...
```json loop
{
  ...
  "model": null,
  "completed_at": "2026-08-01T13:30:00Z"
}
```
...
```json loop
{
  ...
  "acceptance": [
    ...
    "AC-3: python3 -c \"import yaml; d=yaml.safe_load(open('policy.yml')); assert 'freeze' in d; print('freeze config OK')\" EXIT=0"
  ],
  "blocked_by": [],
  "model_hint": "qwen3-max",
  "budget": 1.0,
...
```json loop
{
  ...
  "acceptance": [
    ...
    "AC-4: cat conductor/tick.py 含 freeze 检查逻辑（grep 'frozen' 或 'FROZEN' 命中率 > 0）EXIT=0"
  ],
  "blocked_by": [
    "W0-4"
  ],
...
```json loop
{
  ...
  "paths": [
    ".github/CODEOWNERS",
    "settings/loop-main-protection.json",
...
> ⚠ **路径交叉说明**：
> - W0-3 与 W0-5 在 `conductor/tick.py` 上有交叉。W0-5 已 `blocked_by: ["W0-2", "W0-3"]`，W0-3 完成前 W0-5 不会启动。W0-3 修复根因 + freeze 逻辑，W0-5 新增功能（digest 生成）。
> - W0-3 与 W0-4 在 `.github/workflows/conductor.yml` 上有交叉。W0-3 已 `blocked_by: ["W0-4"]`，W0-4 完成前 W0-3 不会启动。W0-4 修改 cron 频率 + 加 frozen 守卫，W0-3 修改 tick 调用参数 + 注入 freeze 环境变量。
> - 两张卡在各自 `forbid_paths` 中互斥对方的核心文件，防止越界修改。
...
5. `gh api repos/Cloudbird-Software/product-x/rulesets/19949520 --jq '.rules[] | select(.type=="pull_request") | .parameters.require_code_owner_review'` → `true`
...
- N2：`freeze.all=true` → tick 退出 0、日志 `FROZEN`、无写操作（行为级验证，AC-4 只检查代码存在性）
- N3：liveness 阈值临时改 1h → 开 Incident
- N4：非法 cron → actionlint 红

## W0-6 关于 N5 的特殊说明

W0-6 修改 `settings/*.json` 属分支保护 ruleset 真源（CHARTER N5：不自动修正，只检测漂移开 Incident）。
- **技术可行性**：`gate_paths.py` 仅检查卡片自身 `forbid_paths`，不执行 AGENTS.md "不可改的路径"列表，因此卡片可合法包含 `settings/*.json`。
- **操作流程**：① impl 写入 settings/*.json 新值 → ② 人类在 GitHub Settings 页面手动应用 ruleset 变更 → ③ PR CI 的 `gate/settings-roundtrip` 对比本地与线上，人类应用后应一致。
- **预期过渡态**：若 ② 滞后于 ①，`gate/settings-roundtrip` 会检测漂移并 FAIL——这是 N5 设计的预期行为（漂移可见、不自动修）。人类应用后漂移自动消失。
