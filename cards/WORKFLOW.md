...
> **⚠️ ARCHIVED — 本流程已随 cards/ 于 2026-07-30 冻结（R10-5）。**
> 下方暂行流程仅作历史记录保留，**不再具有权威性**，新会话不得据此改 markdown 字段推进状态机。
> 工单真源已切到 **product-x issues** 的 ```json loop``` 块，状态推进只走 loopd（CAS + 租约）。
> 入口提示词见 [prompts/P-continue.md](../prompts/P-continue.md)。

> ~~暂行流程继续生效~~ @2026-07-30 修订（已被上方冻结声明取代）
>
